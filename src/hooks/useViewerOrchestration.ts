...
export function useViewerOrchestration(options: ViewerOrchestrationOptions): ViewerOrchestrationState {
  ...
  const openerRetryTimersRef = useRef<number[]>([]);
  ...
  const clearOpenerRetryTimers = useCallback(() => {
    if (typeof window === "undefined") {
      openerRetryTimersRef.current = [];
      return;
    }
    openerRetryTimersRef.current.forEach((timerId) => window.clearTimeout(timerId));
    openerRetryTimersRef.current = [];
  }, []);
  ...
  const attachStream = useCallback(
    ...
    [appendDebug, clearOpenerRetryTimers, onStreamReceived, updateStatus, videoRef],
  );

  const tryAttachStreamFromOpener = useCallback(
    (streamId: string, attempt = 0): boolean => {
      if (typeof window === "undefined") {
        return false;
      }
      const candidate = getStreamFromOpener(streamId);
      if (candidate) {
        if (attempt > 0) {
          appendDebug(`retrieved stream ${streamId} from opener after ${attempt + 1} tries`);
        }
        attachStream(candidate, streamId);
        return true;
      }
      if (attempt >= 4) {
        appendDebug(`stream ${streamId} unavailable from opener after retries`);
        return false;
      }
      const delay = Math.min(1000, 200 * (attempt + 1));
      const timerId = window.setTimeout(() => {
        tryAttachStreamFromOpener(streamId, attempt + 1);
      }, delay);
      openerRetryTimersRef.current.push(timerId);
      return false;
    },
    [appendDebug, attachStream],
  );
  ...
  const handleSessionMessage = useCallback(
      ...
      switch (payload.type) {
        ...
        case "deliver-stream":
          if (payload.stream) {
            ...
            const streamId = payload.streamId ?? PRIMARY_STREAM_ID;
            const attached = tryAttachStreamFromOpener(streamId);
            if (!attached && payload.streamId) {
              requestStream(payload.streamId);
            }
          }
          break;
    ...
    [appendDebug, attachStream, onStreamEnded, requestStream, stopPlaybackStream, tryAttachStreamFromOpener, updateStatus],
  );
