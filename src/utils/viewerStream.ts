...
export function captureCanvasStream(
  ...
  try {
    ...
    if ('captureStream' in canvas) {
      ...
      // Proactively request a frame so newly attached viewers receive pixels even if
      // the canvas content is static when capture starts.
...
export function sendStreamToViewer(
  ...
  try {
    ...
    // Kick the capture stream so the viewer receives a fresh frame even if the
    // canvas is currently static.
    requestStreamFrame(stream);
    ...
    // Always send a non-transfer fallback so older browsers can fetch the
    // stream from the registry on the presenter window.
    postSessionMessage(
      targetWindow,
      {
        type: 'deliver-stream',
