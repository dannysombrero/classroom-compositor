...
export default function JoinPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/join-codes/${cleaned.replace(/-/g, "")}`);
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-6">
      ...
      {error && <div className="w-full max-w-md rounded-md border border-red-200 bg-red-50 text-red-700 px-3 py-2 text-sm">{error}</div>}
      <form onSubmit={onSubmit} className="flex items-center gap-2 w-full max-w-md">
        <input value={code} onChange={(e)=>setCode(e.target.value.toUpperCase())} placeholder="7D9-K2F" className="flex-1 border rounded px-3 py-2 font-mono" maxLength={9}/>
