  ...
  export class MediaPipeSegmenter implements ISegmenter {
    ...
    setVideo(input: HTMLVideoElement): void {
      ...
      const step = async () => {
        ...
        // Check if mask hasn't updated in a while (possible freeze)
        const timeSinceUpdate = Date.now() - this.lastMaskUpdate;
        if (timeSinceUpdate > 3000 && this.errorCount === 0) {
          console.warn('[MediaPipe] Mask not updating for 3s, possible freeze');
          this.errorCount++;
        }
        ...
        try {
          ...
          if (this.errorCount > 10) {
            console.error('[MediaPipe] Too many errors, stopping segmentation');
            this.running = false;
            return;
          }
        }
